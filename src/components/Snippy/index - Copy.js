...
export default class Snippy extends Component {
    ...
    onSaveSnippet(e)
    {
        debugger;
        if (e.isNew == 0) {
            this.SaveItem(e.title, e.text, e.tags);
        }
        else {
            this.setState({currentId: e.id});
            this.UpdateItem(e.title, e.text, e.tags);
        }
    ...
    SaveItem(title, text, tags) {
        ...
        this.SaveItemToDB(title, text, tags.join(','), userName, strDateTime, id, this.state.currentUserId, true);


    }

    SaveItemToDB(title, text, tags, userName, strDateTime, id, cId, toServer) {
        var data = {
            id: id,
            title: title,
            text: text,
    ...
    UpdateItem(title, text, tags) {
        ...
        localStorage.setItem(this.state.currentId, JSON.stringify(data));
        this.PostAjaxDataToApp("/saveSnippet", data, function (e) {
            //ViewMySnippets();
            //getAllData();
